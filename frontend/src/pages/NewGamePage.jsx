...
export default function NewGamePage() {
  ...
  useEffect(() => {
    api.getDecks().then(setAllDecks).catch(() => setError(tr('newGamePage.loadDecksError')))
  }, [])
  ...
  const submit = async () => {
    if (filledSlots.length < 3) { setError(tr('newGamePage.minPlayersError')); return }
    if (!winnerId) { setError(tr('newGamePage.selectWinnerError')); return }

    // Controlla duplicati giocatore
    const uids = filledSlots.map(s => s.userId)
    if (new Set(uids).size !== uids.length) { setError(tr('newGamePage.duplicatePlayerError')); return }
    ...
    try {
      ...
      if (podCtx) {
        ...
        fireConfetti()
        toast(tr('newGamePage.podResultSaved'), 'success')
        setTimeout(() => navigate(`/evento/${podCtx.eventId}`), 700)
        return
      }
      fireConfetti()
      toast(tr('newGamePage.gameLogged'), 'success')
      setTimeout(() => navigate('/'), 700)
    } catch (err) {
      setError(err.error || tr('newGamePage.saveError'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div>
      <div style={{ fontSize: 20, fontWeight: 800, marginBottom: '1.25rem', color: t.text }}>{podCtx ? tr('newGamePage.podResultTitle') : tr('newGamePage.newGameTitle')}</div>
      {podCtx && (
        <div style={{ ...card, fontSize: 13, color: t.textSub }}>
          {tr('newGamePage.podInfoBox')}
        </div>
      )}

      {allDecks.length === 0 && !error && (
        <div style={{ ...card, color: t.textSub, fontSize: 14 }}>
          <Trans i18nKey="newGamePage.noDecksFound" components={{ strong: <strong /> }} />
        </div>
      )}

      {/* Data partita */}
      <div style={card}>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8, color: t.text }}>{tr('newGamePage.gameDateLabel')}</div>
        <input
          type="date"
          value={playedAt}
        ...
        />
        {podCtx && <div style={{ fontSize: 12, color: t.textMuted, marginTop: 6 }}>{tr('newGamePage.presetEventDate')}</div>}
      </div>

      {/* Slot giocatori */}
      <div style={card}>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: t.text }}>{tr('newGamePage.playersAtTable')}</div>
        {slots.map((slot, i) => {
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
                <select style={{ ...sel, opacity: podCtx ? 0.7 : 1 }} value={slot.userId} onChange={e => updateSlot(i, 'userId', e.target.value)} disabled={!!podCtx}>
                  <option value="">{tr('newGamePage.playerPlaceholder')}</option>
                ...
                <select style={sel} value={slot.deckId} onChange={e => updateSlot(i, 'deckId', e.target.value)} disabled={!slot.userId}>
                  <option value="">{tr('newGamePage.deckPlaceholder')}</option>
      ...
      {filledSlots.length >= 2 && (
        <div style={card}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: t.text }}>{tr('newGamePage.whoWon')}</div>
      ...
      {winnerId && losers.length >= 2 && (
        <div style={card}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, color: t.text }}>
            {tr('newGamePage.eliminationOrderLabel')} <span style={{ fontWeight: 400, color: t.textMuted }}>{tr('newGamePage.optionalTag')}</span>
          </div>
          <div style={{ fontSize: 12, color: t.textSub, marginBottom: 12 }}>
            {tr('newGamePage.eliminationOrderHint', { first: ordinal(1, locale) })}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: elimOrder.length ? 14 : 0 }}>
            {losers.map((s, i) => {
              ...
              return (
                ...
                >
                  {picked && <span style={{ fontSize: 11, fontWeight: 800 }}>{ordinal(filledSlots.length - pos, locale)}</span>}
                  {getUserName(s.userId)} · {getDeckName(s.deckId)}
                </button>
              )
            })}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12, color: elimOrder.length === losers.length ? t.win : t.textMuted }}>
                {elimOrder.length === losers.length
                  ? tr('newGamePage.rankingComplete')
                  : tr('newGamePage.orderedCount', { done: elimOrder.length, total: losers.length })}
              </span>
              <button onClick={() => setElimOrder([])} style={{ fontSize: 11, color: t.primary, background: 'none', border: 'none', cursor: 'pointer' }}>
                {tr('newGamePage.resetOrder')}
              </button>
            </div>
          )}
        </div>
      ...
      {winnerId && losers.length >= 1 && (
        <div style={card}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, color: t.text }}>
            {tr('newGamePage.eliminationsLabel')} <span style={{ fontWeight: 400, color: t.textMuted }}>{tr('newGamePage.optionalTag')}</span>
          </div>
          <div style={{ fontSize: 12, color: t.textSub, marginBottom: 12 }}>
            {tr('newGamePage.eliminationsHint')}
          </div>
          {losers.map((s, i) => {
            const key = slotKey(s)
            return (
      ...
      {/* Note */}
      <div style={card}>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8, color: t.text }}>{tr('newGamePage.notesLabel')} {tr('newGamePage.optionalTag')}</div>
        <input
          ...
          placeholder={tr('newGamePage.notesPlaceholder')}
          value={notes}
      ...
      >
        {saving ? tr('newGamePage.saving') : tr('newGamePage.saveGame')}
      </button>
    </div>
  )
}
