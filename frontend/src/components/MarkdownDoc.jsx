...
// Parser markdown minimale (estratto da GuidaPage): copre quanto basta per i
// documenti generati dal repo (heading, paragrafi, liste, tabelle, quote,
// hr, immagini) senza tirarsi dietro una libreria intera per pochi file statici.
...
function inline(text, t) {
  const html = text
    ...
    .replace(/\[([^\]\n]+)\]\(([^)\n]+)\)/g, '<a href="$2" style="color:' + t.primary + '" target="_blank" rel="noopener noreferrer">$1</a>')
  return { __html: html }
}

// Renderizza markdown grezzo dentro una card in stile app. `content` è il
// testo del file .md (import Vite con `?raw`); `title` compare in cima se
// il markdown non ha già un H1 (i documenti legali usano il proprio H1).
    ...
    <div style={cardStyle}>
      {blocks.map((block, idx) => {
        switch (block.t) {
          ...
          case 'h': {
            // L'H1 non si renderizza qui: ogni pagina che usa MarkdownDoc ha
            // già il proprio titolo fuori dalla card (vedi GuidaPage/PrivacyPage/
            // TermsPage) — il markdown resta comunque leggibile con l'H1 se
            // aperto direttamente su GitHub.
