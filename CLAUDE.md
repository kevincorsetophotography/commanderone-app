...
Estrazioni di supporto nate durante la conversione, utili anche fuori da questa fase: `lib/timeAgo.js` (tempo relativo), `lib/ordinal.js` (piazzamento "1°" it / "1st" en, consolidato ovunque), `bracketLabel()` in `lib/brackets.js` (le label bracket erano hardcoded in italiano nella costante — stesso problema di `lib/seasons.js`, risolto qui traducendo a render-time invece che nella costante), `categorizeCard()` in `lib/scryfall.js` ora ritorna chiavi interne stabili in inglese invece di stringhe già in italiano. Namespace `common` (`back`/`edit`/`delete`/`cancel`/`loading`/`save`) e `nav` per la UI generica riusabile.

**Nota strutturale non risolta**: `lib/seasons.js` (`LABELS` stagioni, "Gennaio–Aprile" ecc.) ha lo stesso problema già risolto per bracket/categorie — nessuna pagina convertita la usava direttamente per il rendering della label (solo `s.label` già pronto dai dati), quindi non è mai emerso un punto concreto da correggere; da affrontare se/quando emerge. `lib/archetypes.js` lasciato in inglese di proposito: sono termini di gergo Magic (Aggro, Control, Combo, Stax...) usati così a livello internazionale.
