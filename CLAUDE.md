...
- **i18n Fase 2 — completa** (frontend applicativo, backend, guida utente e documenti legali, vedi sotto): tutto il prodotto è ormai bilingue IT/EN, incluse `GUIDA_UTENTE.en.md`/`PRIVACY_POLICY.en.md`/`TERMINI_SERVIZIO.en.md` scelte a runtime in base a `i18n.language`. Restano da compilare solo i segnaposto legali (vedi "Prossimi").

### Prossimi
1. **Segnaposto legali** in `PRIVACY_POLICY.md`/`TERMINI_SERVIZIO.md` (e relative `.en.md`, stessi placeholder): titolare del trattamento, email di contatto, provider di hosting/database scelto, legge applicabile/foro competente. Da compilare — idealmente con revisione di chi si occupa di conformità GDPR nella propria giurisdizione — **prima che l'app sia raggiungibile da utenti reali**. Bloccante per la pubblicazione, non per lo sviluppo.
2. **Deploy**: non ancora configurato per questo progetto (nessun Railway/Vercel collegato). Quando si è pronti a uscire dal locale, replicare il pattern di Commanderone (vedi sezione Stack & deploy in cima al file).
3. Wiring UI della paginazione partite (vedi sopra) — solo se/quando lo storico di un gruppo reale inizia a diventare pesante da caricare tutto insieme. Non urgente.
...
**Contenuti legali/guida tradotti**: `PRIVACY_POLICY.en.md`/`TERMINI_SERVIZIO.en.md`/`GUIDA_UTENTE.en.md` affiancano gli originali italiani (stessa struttura, stessi segnaposto `[tra parentesi quadre]` ancora da completare — la traduzione non risolve i placeholder legali, li riporta identici). `PrivacyPage`/`TermsPage`/`GuidaPage` scelgono il file `.md` giusto in base a `i18n.language` invece di importarne uno fisso; `MarkdownDoc.jsx` resta agnostico alla lingua (renderizza il testo che riceve). Chrome di queste tre pagine (bottone indietro, titolo) tradotto con `common.back` e `auth.register.privacyLink`/`termsLink`/`docsPages.guideTitle` — era rimasto fuori dalla conversione UI iniziale perché vive negli stessi file del contenuto.
