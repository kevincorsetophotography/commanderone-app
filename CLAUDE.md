...
- **Deploy**: Railway (backend + Postgres) + Vercel (frontend), stesso pattern di Commanderone (`backend/railway.json`, `frontend/vercel.json`, `start:prod` = `prisma db push --skip-generate --accept-data-loss && index.js`; niente più `ensureAdmin`/migrazioni legacy, il bootstrap admin non serve più — l'ADMIN di un gruppo nasce da chi lo crea). Guida completa in `DEPLOY.md` (root del repo) — progetti Railway/Vercel **separati** da quelli di Commanderone, anche condividendo gli stessi account. Provisioning effettivo (creare i progetti, collegare il repo, impostare le variabili) ancora da fare manualmente sui due dashboard — vedi Roadmap.
- **Node 18** in locale.
...
- `GUIDA_UTENTE.md` riscritta da zero, generica (nessuno screenshot ancora — quelli di Commanderone erano specifici di Villastellone). Stesso trattamento per `PRIVACY_POLICY.md`/`TERMINI_SERVIZIO.md` (vedi Roadmap: titolare/contatto/legge applicabile compilati, resta il solo placeholder hosting in attesa del deploy).
...
- Icona app reale, Privacy Policy + Termini di Servizio.
...
- **i18n Fase 2 — completa** (frontend applicativo, backend, guida utente e documenti legali, vedi sotto): tutto il prodotto è ormai bilingue IT/EN, incluse `GUIDA_UTENTE.en.md`/`PRIVACY_POLICY.en.md`/`TERMINI_SERVIZIO.en.md` scelte a runtime in base a `i18n.language`.
- **Segnaposto legali compilati** in `PRIVACY_POLICY.md`/`TERMINI_SERVIZIO.md` (e relative `.en.md`): titolare del trattamento **Kevin Corseto**, contatto **ckappa97@gmail.com**, legge applicabile italiana (foro di residenza del titolare, salve le competenze inderogabili a tutela del consumatore). Resta aperto solo il placeholder hosting/database (sezione 5.5 della Privacy Policy) — dipende dalla scelta del provider di deploy, vedi "Prossimi". La clausola di limitazione di responsabilità nei Termini (sezione 10) resta comunque segnata come "da rivedere con un legale": è un giudizio legale, non un dato da compilare.
- **`DEPLOY.md`**: guida passo-passo Railway (backend + Postgres) + Vercel (frontend), adattata dal `DEPLOY.md` di Commanderone alle differenze architetturali (niente `INVITE_CODE`/`ADMIN_USERNAME`/`ADMIN_PASSWORD` — registrazione e creazione Gruppo sono self-service; aggiunte le variabili opzionali `RESEND_API_KEY`/`MAIL_FROM`/`GROQ_API_KEY`/`JUDGE_DAILY_LLM_CAP`). Include un `JWT_SECRET` già generato pronto all'uso. `backend/railway.json` e `frontend/vercel.json` già presenti nel repo, nessuna modifica necessaria. Corretto anche un commento non più veritiero in `backend/.env.example` (diceva che senza `GROQ_API_KEY` l'endpoint rispondesse 503 — in realtà passa al fallback gratuito, come da comportamento reale del codice).

### Prossimi
1. **Deploy — provisioning manuale**: config e guida pronti (`DEPLOY.md`, `railway.json`, `vercel.json`, `start:prod`), ma i progetti Railway/Vercel non sono ancora stati creati sui dashboard — è un passaggio a clic che solo chi ha accesso agli account può fare. Una volta fatto, completare di conseguenza il placeholder hosting nella Privacy Policy (sezione 5.5, IT+EN) col provider/region effettivi.
2. Wiring UI della paginazione partite (vedi sopra) — solo se/quando lo storico di un gruppo reale inizia a diventare pesante da caricare tutto insieme. Non urgente.
3. Prima di rendere l'app raggiungibile da utenti reali, far verificare Privacy Policy/Termini da chi si occupa di conformità GDPR nella propria giurisdizione — i segnaposto sono compilati ma restano una bozza tecnica, non una revisione legale.
...
**Contenuti legali/guida tradotti**: `PRIVACY_POLICY.en.md`/`TERMINI_SERVIZIO.en.md`/`GUIDA_UTENTE.en.md` affiancano gli originali italiani (stessa struttura; entrambe le lingue avevano gli stessi segnaposto `[tra parentesi quadre]`, poi compilati in entrambe — vedi sopra — tranne l'hosting, ancora in sospeso in entrambe). `PrivacyPage`/`TermsPage`/`GuidaPage` scelgono il file `.md` giusto in base a `i18n.language` invece di importarne uno fisso; `MarkdownDoc.jsx` resta agnostico alla lingua (renderizza il testo che riceve). Chrome di queste tre pagine (bottone indietro, titolo) tradotto con `common.back` e `auth.register.privacyLink`/`termsLink`/`docsPages.guideTitle` — era rimasto fuori dalla conversione UI iniziale perché vive negli stessi file del contenuto.
