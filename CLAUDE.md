...
**Già convertiti** (oltre alla Fase 1): componenti condivisi `NotificationBell`/`GameSocial`/`DeckListPanel`/`BracketBadge`, `FeedPage` (la pagina più vista), `VerifyEmailPage`/`ResetPasswordPage` (flusso auth rimasto fuori dalla Fase 1 per errore), `GamePage`, `GiocaPage`, `DecksPage`, `EventsPage`, `JudgePage`, `GruppoPage`, `EventDetailPage`.
